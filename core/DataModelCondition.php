...
class DataModelCondition
{
    ...
    public function __construct($field, $operate, $value, $addition = null)
    {
        ...
        if ($this->operate === self::OP_LIKE || $this->operate === self::OP_NOT_LIKE) {
            $this->value = LibPDO::dryQuote($this->value);
            switch ($addition) {
    ...
    public function makeConditionSQL()
    {
        switch ($this->operate) {
            ...
            case self::OP_NULL_SAFE_EQUAL:
                return "`{$this->field}` " . $this->operate . " " . LibPDO::dryQuote($this->value);
                break;
            case self::OP_IS:
            case self::OP_IS_NOT:
            ...
            case self::OP_NOT_BETWEEN:
                return "`{$this->field}` " . $this->operate . " " . LibPDO::dryQuote($this->value[0]) . " AND " . LibPDO::dryQuote($this->value[1]);
                break;
            default:
